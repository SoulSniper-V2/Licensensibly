  ...
  return (
    <div className="mx-auto max-w-[1200px] px-6 py-8 space-y-6">
      <div className="flex flex-wrap gap-4 items-start justify-between">
        ...
        <Link href="/settings" className="mono text-xs border border-zinc-200 rounded-full px-3 py-1.5 bg-white hover:bg-zinc-50">Settings →</Link>
      </div>

      {/* Consolidated header bar — replaces static Company Profile card */}
      <div className="rounded-2xl border border-zinc-200 bg-white p-4 flex flex-wrap gap-3 items-center justify-between">
        <div className="flex flex-wrap gap-3 items-center">
          <span className="mono text-xs tracking-[0.14em] text-zinc-500 font-semibold">COMPANY</span>
          <select value={companyId} onChange={e=> setCompanyId(e.target.value)} className="rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-sm font-medium">
            {MOCK_COMPANIES.map(c=> <option key={c.id} value={c.id}>{c.legalName}</option>)}
          </select>
          <span className="mono text-xs text-zinc-500 hidden sm:inline">{company.licenses.length} licenses • {company.licenses.map(l=> `${l.state} ${l.classification}`).join(" • ")}</span>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={()=> setShowIntakePanel(true)} className="mono text-xs border border-zinc-200 rounded-full px-4 py-1.5 bg-white hover:bg-zinc-50">Edit intake → panel</button>
        </div>
      </div>
      ...
      {showCompanyPanel && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-[480px] max-w-[90vw] bg-white border-l border-zinc-200 shadow-2xl p-6 space-y-4 overflow-auto">
            <div className="flex items-center justify-between">
              <h3 className="mono text-xs tracking-[0.14em] text-zinc-500 font-semibold">COMPANY PROFILE — PANEL</h3>
              <button onClick={()=>setShowCompanyPanel(false)} className="rounded-full border border-zinc-200 mono text-xs px-3 py-1.5 hover:bg-zinc-50">Close</button>
            </div>
            <div className="rounded-2xl bg-zinc-900 text-white p-5 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5 pt-2">
                {company.licenses.map(l=> <span key={l.id} className="rounded-full bg-white/10 border border-white/20 px-2.5 py-1 text-xs">{l.state} {l.trade} {l.classification}</span>)}
              </div>
            </div>
            <div className="space-y-3">
              <h4 className="mono text-xs font-semibold text-zinc-700">Licenses</h4>
              {company.licenses.map(l=> (
                <div key={l.id} className="rounded-xl border border-zinc-200 p-3 mono text-xs">
                  <div className="font-semibold text-zinc-900">{l.state} — {l.trade} — {l.classification}</div>
                  <div className="text-zinc-500 mt-1">{l.licenseNumber} • Qualifier: {l.qualifier}</div>
                  <div className="text-zinc-500">Issued {l.issuedDate} • Exp {l.expiryDate} • {l.status}</div>
                </div>
              ))}
            </div>
            <div className="rounded-xl bg-zinc-50 border border-zinc-200 p-4 mono text-xs text-zinc-600">
              <div className="font-semibold text-zinc-900">Qualifiers</div>
              <div className="mt-1">{company.qualifiers.map(q=> `${q.name} (${q.trades.join(", ")} — ${q.states.join("/")})`).join(" • ")}</div>
              <div className="mt-3 font-semibold text-zinc-900">Insurance</div>
            ...
            <Link href="/settings" onClick={()=>setShowCompanyPanel(false)} className="block text-center rounded-full bg-white border border-zinc-200 mono text-xs font-semibold py-2.5 hover:bg-zinc-50">Open full Settings →</Link>
            <p className="mono text-xs text-zinc-500">This panel consolidates the previously static Company Profile card into a popup per your 4:41 AM request. Same data, ~60% less vertical chrome on Check.</p>
          </div>
        </div>
      )}
      ...
      {showIntakePanel && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-[520px] max-w-[90vw] bg-white border-l border-zinc-200 shadow-2xl p-6 space-y-4 overflow-auto">
            <div className="flex items-center justify-between">
              <h3 className="mono text-xs tracking-[0.14em] text-zinc-500 font-semibold">PROJECT INTAKE — PANEL</h3>
              <button onClick={()=>setShowIntakePanel(false)} className="rounded-full border border-zinc-200 mono text-xs px-3 py-1.5 hover:bg-zinc-50">Close</button>
            </div>
            <p className="mono text-xs text-zinc-600">Full intake as a slide-over — proves the “everything in popups/panels” direction. You can keep the inline form or switch to panel-only later.</p>
            <label className="block mono text-xs font-semibold text-zinc-700">Project title<input value={form.title} onChange={e=> setForm({...form, title:e.target.value})} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm" /></label>
            <div className="grid grid-cols-2 gap-3">
              <label className="block mono text-xs font-semibold text-zinc-700">City<input value={form.city} onChange={e=> setForm({...form, city:e.target.value})} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm" /></label>
              <label className="block mono text-xs font-semibold text-zinc-700">State<select value={form.state} onChange={e=> setForm({...form, state:e.target.value as any})} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm font-semibold"><option value="NC">NC</option><option value="SC">SC</option><option value="VA">VA</option></select></label>
            ...
            <label className="block mono text-xs font-semibold text-zinc-700">Address<input value={form.address} onChange={e=> setForm({...form, address:e.target.value})} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm" /></label>
            <label className="block mono text-xs font-semibold text-zinc-700">County<input value={form.county} onChange={e=> setForm({...form, county:e.target.value})} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm" /></label>
            <div className="grid grid-cols-2 gap-3">
              <label className="block mono text-xs font-semibold text-zinc-700">Trade<select value={form.trade} onChange={e=> setForm({...form, trade:e.target.value as Trade})} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm"><option value="electrical">Electrical</option><option value="hvac">HVAC</option><option value="fire-protection">Fire Protection</option></select></label>
              <label className="block mono text-xs font-semibold text-zinc-700">Value $<input type="number" value={form.contractValue} onChange={e=> setForm({...form, contractValue: Number(e.target.value)})} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm" /></label>
            </div>
            <label className="block mono text-xs font-semibold text-zinc-700">Scope<textarea value={form.scope} onChange={e=> setForm({...form, scope:e.target.value})} rows={3} className="mt-1 w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm" /></label>
