...
export default function CompaniesPage() {
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-6 py-8 space-y-6">
      ...
      {showAdd && (
        <div className="border bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 space-y-3">
          <h3 className="text-sm font-black">New company</h3>
          <div className="grid md:grid-cols-3 gap-3">
            <label className="text-xs font-bold">Legal name<input value={draft.legalName||""} onChange={e=> setDraft({...draft, legalName: e.target.value})} className="mt-1 w-full border border border-zinc-200 dark:border-zinc-800 px-3 py-2 text-sm" placeholder="Acme Electrical LLC" /></label>
            <label className="text-xs font-bold">Entity type
              <select value={draft.entityType} onChange={e=> setDraft({...draft, entityType: e.target.value as any})} className="mt-1 w-full border border border-zinc-200 dark:border-zinc-800 px-3 py-2 text-sm">
            ...
            <label className="text-xs font-bold">Home state(s) comma-separated<input value={(draft.incorporatedStates||[]).join(",")} onChange={e=> setDraft({...draft, incorporatedStates: e.target.value.split(",").map(s=> s.trim().toUpperCase()).filter(Boolean)})} className="mt-1 w-full border border border-zinc-200 dark:border-zinc-800 px-3 py-2 text-sm" placeholder="NC" /></label>
          </div>
          <label className="text-xs font-bold">Foreign qualifications (comma-separated)<input value={(draft.foreignQualifications||[]).join(",")} onChange={e=> setDraft({...draft, foreignQualifications: e.target.value.split(",").map(s=> s.trim().toUpperCase()).filter(Boolean)})} className="mt-1 w-full border border border-zinc-200 dark:border-zinc-800 px-3 py-2 text-sm" placeholder="SC, VA" /></label>
      ...
      <div className="grid lg:grid-cols-[300px_1fr] gap-6">
          ...
          {companies.map(c=> (
            <button key={c.id} onClick={()=> setSelectedId(c.id)} className={`w-full text-left border border p-4 transition ${selectedId===c.id ? "bg-white dark:bg-zinc-900 text-black border-slate-900" : "bg-white dark:bg-zinc-900 border-slate-200 hover:border-slate-300"}`}>
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {c.licenses.map(l=> <span key={l.id} className={`text-[11px] px-2 py-0.5 border font-semibold ${selectedId===c.id ? "bg-white dark:bg-zinc-900/15 border border-white/20" : "bg-black border border-zinc-200 dark:border-zinc-800"}`}>{l.state}:{l.classification}</span>)}
        ...
        <div className="space-y-4">
          <div className="border bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 flex flex-wrap justify-between gap-3">
              ...
              <Link href={`/check`} className="border bg-amber-400 text-zinc-900 dark:text-zinc-100 font-bold px-4 py-2 text-sm h-fit">Check eligibility →</Link>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              ) : <div className="border bg-black border border-dashed border-slate-300 p-6 text-sm text-zinc-500 dark:text-zinc-400 text-center">No licenses — eligibility engine will block all NC/SC/VA projects &gt; $40k/$5k. Add a license to go conditional/eligible.</div>}

              <div className="grid md:grid-cols-2 gap-4">
                <div className="border bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4">
