    ...
    <div className="mx-auto max-w-[1400px] px-4 md:px-6 py-8 space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">MVP #1 — The credential graph: <code className="bg-zinc-800 px-1 rounded">jurisdiction × trade × value × credentials → eligibility</code>. Licenses, qualifiers, insurance, foreign qualifications. Engine reads this on every check.</p>
        </div>
      ...
      {showAdd && (
        <div className="border bg-zinc-900 border border-zinc-800 p-5 space-y-3">
          <h3 className="text-sm font-black">New company</h3>
          <div className="grid md:grid-cols-3 gap-3">
            <label className="text-xs font-bold">Legal name<input value={draft.legalName||""} onChange={e=> setDraft({...draft, legalName: e.target.value})} className="mt-1 w-full border border border-zinc-800 px-3 py-2 text-sm" placeholder="Acme Electrical LLC" /></label>
            <label className="text-xs font-bold">Entity type
              <select value={draft.entityType} onChange={e=> setDraft({...draft, entityType: e.target.value as any})} className="mt-1 w-full border border border-zinc-800 px-3 py-2 text-sm">
            ...
            <label className="text-xs font-bold">Home state(s) comma-separated<input value={(draft.incorporatedStates||[]).join(",")} onChange={e=> setDraft({...draft, incorporatedStates: e.target.value.split(",").map(s=> s.trim().toUpperCase()).filter(Boolean)})} className="mt-1 w-full border border border-zinc-800 px-3 py-2 text-sm" placeholder="NC" /></label>
          </div>
          <label className="text-xs font-bold">Foreign qualifications (comma-separated)<input value={(draft.foreignQualifications||[]).join(",")} onChange={e=> setDraft({...draft, foreignQualifications: e.target.value.split(",").map(s=> s.trim().toUpperCase()).filter(Boolean)})} className="mt-1 w-full border border border-zinc-800 px-3 py-2 text-sm" placeholder="SC, VA" /></label>
          <div className="flex gap-2">
            <button onClick={addCompany} className="border bg-amber-400 text-white font-bold px-5 py-2 text-sm">Create</button>
      ...
      <div className="grid lg:grid-cols-[300px_1fr] gap-6">
          ...
          {companies.map(c=> (
            <button key={c.id} onClick={()=> setSelectedId(c.id)} className={`w-full text-left border border p-4 transition ${selectedId===c.id ? "bg-white text-black border-slate-900" : "bg-white border-slate-200 hover:border-slate-300"}`}>
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {c.licenses.map(l=> <span key={l.id} className={`text-[11px] px-2 py-0.5 border font-semibold ${selectedId===c.id ? "bg-white/15 border border-white/20" : "bg-black border border-zinc-800"}`}>{l.state}:{l.classification}</span>)}
        ...
        <div className="space-y-4">
          <div className="border bg-zinc-900 border border-zinc-800 overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 flex flex-wrap justify-between gap-3">
              ...
              <Link href={`/check`} className="border bg-amber-400 text-white font-bold px-4 py-2 text-sm h-fit">Check eligibility →</Link>
            </div>
            ...
            <div className="p-6 space-y-4">
                  ...
                  {selected.licenses.map((l: CompanyLicense)=> {
                    ...
                    return (
                      <div key={l.id} className={`border border p-4 ${nearExpiry ? "bg-amber-50 border-amber-200" : "bg-black border-slate-200"}`}>
                        ...
                        <div className="text-xs text-zinc-400 mt-1">Qualifier: <span className="font-semibold text-white">{l.qualifier}</span></div>
                        <div className="text-xs text-zinc-400">Issued {l.issuedDate} • Expires <span className={nearExpiry ? "font-bold text-amber-700" : "font-semibold"}>{l.expiryDate} {nearExpiry ? `(${daysLeft}d)` : ""}</span></div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="border bg-zinc-900 border border-zinc-800 p-4">
              ...
              <div className="border bg-zinc-900 border border-zinc-800 p-4">
                ...
                <div className="text-xs text-zinc-400 mt-1">Registrations: {selected.registrations.length ? selected.registrations.map(r=> `${r.jurisdiction} (${r.type} exp ${r.expiry})`).join(" • ") : "— none (local business registration may be needed to pull permits)"}</div>
              </div>
            </div>
          </div>
