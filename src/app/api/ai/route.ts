...
export async function POST(req: NextRequest) {
  ...
  const systemPreamble = `You are Licensensibly — the inbuilt AI for the Pre-Bid Compliance OS (the app is called Licensensibly). You help specialty contractors (electrical/HVAC/fire-protection) decide if they can legally bid jobs in NC/SC/VA. You EXPLAIN rules and cite official sources; the deterministic engine DECIDES eligibility. Never hallucinate license URLs — if unsure, say to verify with the NCBEEC / NC Licensing Board / SC LLR / VA DPOR. Keep answers short and ops-friendly. Not legal advice.`;
  ...
  try {
    ...
    if (wantsStream) {
      ...
      return new Response(readable, { headers: { "content-type": "text/plain; charset=utf-8", "cache-control": "no-cache", "x-licensensibly-model": AI_MODEL } });
    } else {
