  ...
  return (
    <div className="mx-auto max-w-[1200px] px-6 py-8 space-y-6">
      ...
      <div className="rounded-2xl border bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 p-4 flex flex-wrap items-center gap-3">
        ...
        <select value={stateFilter} onChange={e=> setStateFilter(e.target.value as any)} className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm font-semibold">
          <option value="all">All states</option><option value="NC">NC</option><option value="SC">SC</option><option value="VA">VA</option>
        </select>
        <select value={tradeFilter} onChange={e=> setTradeFilter(e.target.value as any)} className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm font-semibold">
        ...
        <label className="flex items-center gap-2 text-xs font-bold text-zinc-700">
          Evaluate as
          <select value={companyId} onChange={e=> setCompanyId(e.target.value)} className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-2 text-sm font-semibold">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div className="rounded-2xl border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map(p=>{
          ...
          return (
            <div key={p.id} className="rounded-2xl border bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 overflow-hidden flex flex-col">
              ...
              <div className="mt-auto border-t border-slate-100 p-4 flex gap-2">
                ...
                <a href={`https://www.google.com/maps/search/${encodeURIComponent(p.address+", "+p.city+", "+p.state)}`} target="_blank" className="border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 py-2.5 text-xs font-bold">Map</a>
              </div>
            </div>
          );
        })}
      ...
      {filtered.length===0 && (
        <div className="rounded-2xl border bg-white dark:bg-zinc-900 border-dashed border-slate-300 p-12 text-center">
      ...
      <div className="rounded-2xl border bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 p-6 flex flex-wrap gap-6">
        ...
        <div className="rounded-2xl border bg-white dark:bg-zinc-900/10 border-white/15 p-4 text-xs leading-relaxed text-slate-300 min-w-[280px]">
